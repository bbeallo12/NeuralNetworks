...
namespace Neural_Network_Again
{
    class Program
    {
        ...
        {
            //  This file is mostly just user control
            // set file paths
            string appdata = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "\\Neural Net";
            ...
            if (!Directory.Exists(appdata) || !File.Exists(trainingInputPath) || !File.Exists(scalingOutputPath) || !File.Exists(scalingInputPath) || !File.Exists(targetOutputPath) || !File.Exists(inputPath) || !File.Exists(outputPath) || !File.Exists(weightInputPath) || !File.Exists(weightOutputPath) || !File.Exists(biasInputPath) || !File.Exists(biasOutputPath))
            {
                ...
                {
                    Console.WriteLine("Created Neural Net folder at: \n\t{0}", appdata);
            ...
            else
            {
                ...
                // ask user input whether to train or to use a pretrained network
                loop = true;
                while (loop)
                {
                ...
                else // Training a new Network
                {
                    ...
                    if (importMatrix(trainingInputPath, out trainingInput))
                    {
                        ...
                        if (importMatrix(targetOutputPath, out targetOutput))
                        {
                            ...
                            Console.WriteLine("Input Learning Rate (Example 0.1)");
                            do
                            {
                                temp = double.TryParse(Console.ReadLine(), out learningRate);
                                if (!temp)
                                {
                                ...
                                {
                                    temp = false;
                                    Console.WriteLine("Learning rate must be positive");
                                }
                            } while (!temp);

                            Console.WriteLine("Input Momentum (Example 0.2)");
                            do
                            {
                                temp = double.TryParse(Console.ReadLine(), out momentum);
                                if (!temp)
                                {
                                    Console.WriteLine("Invalid Input");
                                }
                                ...
                                {
                                    temp = false;
                                    Console.WriteLine("Momentum must be positive");
                                }
                            } while (!temp);
                            ...
                            do
                            {
                                NN.backPropogation(trainingInput, targetOutput, learningRate, out error);
                                Console.WriteLine("Current Error: {0,-10:0.00000000}", error);
        ...
        static void exportMatrix(string fileLocation, double[,] Matrix, bool Round)
        {
            ...
            for (int y = 0; y < Matrix.GetLength(1); y++)
            {
                for (int x = 0; x < Matrix.GetLength(0); x++)
                {
                    if (Round)
                    {
                        writer.Write("{0:0.000000} ", Matrix[x, y]);
                    }
                    else
                    {
            ...
            for (int z = 0; z < Matrix.Length; z++)
            {
                ...
                for (int y = 0; y < Matrix[z].GetLength(1); y++)
                {
                    for (int x = 0; x < Matrix[z].GetLength(0); x++)
                    {
                        if (Round)
                        {
                            writer.Write("{0:0.000000} ", Matrix[z][x, y]);
                        }
                        else
                        {
